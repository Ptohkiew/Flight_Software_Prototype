...
static unsigned int run_duration_in_sec = 10;
...
void server(void) {
   ...
   if ((conn = csp_accept(&sock, 10000)) == NULL) {
    csp_print("Connection timeout\n"); // เพิ่มการพิมพ์เมื่อเชื่อมต่อไม่สำเร็จ
    continue;
}
...
void client(void) {
	...
	while (1) {
		...
		csp_conn_t * conn = csp_connect(CSP_PRIO_NORM, server_address, MY_SERVER_PORT2, 1000, CSP_O_NONE);
		if (conn == NULL) {
			/* Connect failed */
